...
class DeepAgent():
    ...
    def build_networks(self):
        state_input = Input(shape=(self.frames_per_state, )+self.history.im_shape, name="state_input")
        weight_input = Input(shape=(self.obj_cnt, ), name="weight_input")
        ...
        y_true = Input(name='y_true', shape=(self.action_count,  self.qvalue_dim(), ))

        mask = Input(name='mask', shape=(self.action_count,  self.qvalue_dim(), ))

        loss_out = Lambda(masked_error, output_shape=(1, ), name='loss')([y_true, y_pred, mask])

        trainable_model = Model(inputs=[weight_input, state_input, y_true, mask], outputs=[loss_out, y_pred], name='trainable_model')
        main_model = Model(inputs=[weight_input, state_input], outputs=y_pred)

        return trainable_model, main_model
